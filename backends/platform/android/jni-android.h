...
public:
	...
	static jobject getActivity() { return _jobj_activity; }

	// VR mode: bootstrap ScummVM from C++ without the full Java lifecycle
	static bool createVR(JNIEnv *env, jobject scummvmObj, jobject assetManager);
...
inline int JNI::writeAudio(JNIEnv *env, jbyteArray &data, int offset, int size) {
	if (!_jobj_audio_track) return size; // VR mode: pretend we wrote it
