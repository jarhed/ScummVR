...
#ifdef USE_OPENXR
	// VR mode: skip SAF init — SAF Java classes can't be loaded from native threads
#else
	if (sdkVersion >= 24) {
		fsFactory.initSAF();
	}
...
#ifdef USE_OPENXR
	// VR mode: skip touch controls, use VR graphics manager
	{
		OpenXRSession *xrSession = new OpenXRSession();
		_graphicsManager = new AndroidVRGraphicsManager(xrSession);
	}
#else
