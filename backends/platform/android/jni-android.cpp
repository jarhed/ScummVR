...
bool JNI::initSurface() {
	// In VR mode, surface management is handled by OpenXR, not Java EGL
	if (!_MID_initSurface) {
		LOGI("initSurface: VR mode, skipping Java surface init");
		// Set dimensions once, don't keep incrementing changeid
		if (egl_surface_width == 0) {
			egl_surface_width = 1920;
			egl_surface_height = 1080;
			egl_bits_per_pixel = 32;
			surface_changeid++;
		}
		return true;
	}
...
void JNI::deinitSurface() {
	// In VR mode, nothing to clean up
	if (!_MID_deinitSurface) {
		_jobj_egl_surface = 0;
		return;
	}
...
int JNI::fetchEGLVersion() {
	// In VR mode, return EGL 1.5 (Quest 2 supports this)
	if (!_MID_eglVersion) {
		_egl_version = 0x00010005; // EGL 1.5
		return _egl_version;
	}
...
bool JNI::createVR(JNIEnv *env, jobject scummvmObj, jobject assetManager) {
	LOGI("JNI::createVR - bootstrapping ScummVM for VR mode");

	assert(!_system);

	_jobj = env->NewGlobalRef(scummvmObj);
...
#define FIND_METHOD(prefix, name, signature) do {                           \
    _MID_ ## prefix ## name = env->GetMethodID(cls, #name, signature);      \
        if (_MID_ ## prefix ## name == 0) {                                 \
            LOGE("Can't find method %s (non-fatal in VR)", #name);          \
        }                                                                   \
    } while (0)
	...
	FIND_METHOD(, getSysArchives, "()[Ljava/lang/String;");
	FIND_METHOD(, getAllStorageLocations, "()[Ljava/lang/String;");
	// VR mode: skip SAF methods (SAFFSTree class may not be loadable from native threads)
	_MID_getNewSAFTree = 0;
	_MID_getSAFTrees = 0;
	_MID_findSAFTree = 0;
	_MID_exportBackup = 0;
	_MID_importBackup = 0;
	// Clear any exception from method resolution above
	if (env->ExceptionCheck()) env->ExceptionClear();

	// VR mode: skip initSurface/deinitSurface/eglVersion - we handle rendering ourselves
	// Set them to 0 so we can check before calling
	_MID_initSurface = 0;
	_MID_deinitSurface = 0;
	_MID_eglVersion = 0;

	env->DeleteLocalRef(cls);

	// VR mode: no EGL Java objects
	_jobj_egl = 0;
	_jobj_egl_display = 0;
	_egl_version = 0;

	// Create AudioTrack via JNI for audio output
	{
		// AudioTrack.getNativeOutputSampleRate(STREAM_MUSIC)
		jclass atClass = env->FindClass("android/media/AudioTrack");
		jmethodID getSampleRate = env->GetStaticMethodID(atClass, "getNativeOutputSampleRate", "(I)I");
		jint sampleRate = env->CallStaticIntMethod(atClass, getSampleRate, 3 /* STREAM_MUSIC */);

		// AudioTrack.getMinBufferSize(rate, CHANNEL_OUT_STEREO, ENCODING_PCM_16BIT)
		jmethodID getMinBuf = env->GetStaticMethodID(atClass, "getMinBufferSize", "(III)I");
		jint bufferSize = env->CallStaticIntMethod(atClass, getMinBuf, sampleRate,
			12 /* CHANNEL_OUT_STEREO */, 2 /* ENCODING_PCM_16BIT */);

		// new AudioTrack(STREAM_MUSIC, sampleRate, CHANNEL_OUT_STEREO, ENCODING_PCM_16BIT, bufferSize, MODE_STREAM)
		jmethodID atCtor = env->GetMethodID(atClass, "<init>", "(IIIIII)V");
		jobject audioTrack = env->NewObject(atClass, atCtor,
			3 /* STREAM_MUSIC */, sampleRate,
			12 /* CHANNEL_OUT_STEREO */, 2 /* ENCODING_PCM_16BIT */,
			bufferSize, 1 /* MODE_STREAM */);

		if (audioTrack && !env->ExceptionCheck()) {
			_jobj_audio_track = env->NewGlobalRef(audioTrack);
			env->DeleteLocalRef(audioTrack);

			// Resolve AudioTrack methods
			_MID_AudioTrack_flush = env->GetMethodID(atClass, "flush", "()V");
			_MID_AudioTrack_pause = env->GetMethodID(atClass, "pause", "()V");
			_MID_AudioTrack_play = env->GetMethodID(atClass, "play", "()V");
			_MID_AudioTrack_stop = env->GetMethodID(atClass, "stop", "()V");
			_MID_AudioTrack_write = env->GetMethodID(atClass, "write", "([BII)I");

			LOGI("AudioTrack created: sample_rate=%d buffer_size=%d", sampleRate, bufferSize);
		} else {
			if (env->ExceptionCheck()) {
				env->ExceptionDescribe();
				env->ExceptionClear();
...
#undef FIND_METHOD
	...
	_asset_archive = new AndroidAssetArchive(assetManager);
	assert(_asset_archive);

	_system = new OSystem_Android(44100, 2048);
	assert(_system);

	g_system = _system;

	LOGI("JNI::createVR - OSystem_Android created, g_system set");
	return true;
}
